...
2. **Governance layer** — the existing input/access/output contracts and guardrails, implemented as MCP servers/gateways (see [`architecture-security-and-guardrails.md`](architecture-security-and-guardrails.md#runtime-interoperability-via-mcp)). Decides, per task, what data (if any) is allowed to leave the proprietary layer, and which sensitivity tier applies — enforced at the MCP boundary regardless of which runtime is attached.
...
Because the MCP layer is what enforces the tier, not the runtime itself, the client isn't locked into one vendor's coding assistant to get this — the same governed gateway works whether the goal/loop is being driven through ChatGPT, Claude, Antigravity, Cursor, or a fully custom factory.
